	...
	if DETAILS_LEVELS[METRICS_DETAILS] >= DETAILS_LEVELS['standard']:
		...
		try:
			with open(CPU_PATH, 'r') as f:
				...
				metrics_dict['cpu_user_seconds'].labels(CONTAINER.name, CONTAINER.id).set(int(cpu_stats_list[1]))
				metrics_dict['cpu_system_seconds'].labels(CONTAINER.name, CONTAINER.id).set(int(cpu_stats_list[3]))
		except FileNotFoundError:
